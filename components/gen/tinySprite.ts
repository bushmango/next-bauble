...
export function tinySprite_render(seed: number, x: CanvasRenderingContext2D) {
  ...
  const R = ()=> (Math.sin(++s + i*i) + 1)*1e9 % 256 | 0; // get a seeded random integer between 0-256
  
  for(i = 32 * 16; i--;)                          // for each sprite (32 rows x 16 columns)
  for(let pass = 4; pass--;)                          // 4 passes, outline left/right and fill left/right
